...
- **Never cache a failed ffmpeg probe permanently.** A single 5s probe timeout during a busy boot (all workflows restarting) once flipped `ffmpegAvailable` to false until the next restart, 503-ing every stream start. Rule: cache probe success forever, re-probe after failure (dedupe with an in-flight promise set before any await); routes should `await probeFfmpeg()` so they self-heal per-request; sampler startup retries 3x/5s.

# E2E-testing RTSP in this workspace

**Why:** background processes started in one agent bash session are reaped when the session exits — `nohup setsid` does NOT survive. Also, `pkill -f <pattern>` kills your own shell if the pattern string appears anywhere in the command line you're running.
**How to apply:** run the whole test inside ONE bash command: start `mediamtx` (binary downloadable from GitHub releases into /tmp; needs a config with `paths: all_others:` or it rejects unconfigured paths) + an ffmpeg lavfi publisher with SHORT GOP (`-g 5`, else HLS start waits ~50s for a keyframe), then curl the API, then kill both. Shell and workflows share the network — 127.0.0.1 works both ways. For browser (runTest) verification, spawn the RTSP infra from the code_execution notebook (`spawn(..., {detached:true})` + unref) — it stays alive across the runTest call, then kill by pid.
- **Headless test Chromium cannot decode H.264** (`MediaSource.isTypeSupported('video/mp4; codecs="avc1..."')` = false, VP9 = true) → hls.js fails with `bufferAddCodecError`. Live HLS playback can NEVER pass in the browser-test agent; verify the dialog/poll UI flow there, but verify actual video via API (playlist + segment fetch + ffprobe the .ts). Real browsers play H.264 fine.
