...
import {
  ...
  Trash2, Smartphone, Link2, FileBarChart, BookOpen, Eye, EyeOff,
} from "lucide-react";
import {
  LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend,
} from "recharts";

// ─── API helpers (cookie-session auth, root-relative /api) ────────────────────
...
export default function MmpPage() {
  ...
  const breakdownQ = useQuery({
    ...
    queryFn: () => api<{ data: BreakdownRow[]; organic: { installs: number; revenueUsd: number } }>(`/stats/breakdown?days=${days}${appFilter}`),
  });
  ...
  const [newLink, setNewLink] = useState({ appId: "", name: "", channel: "facebook_ads", destinationUrl: "" });
  const createLink = useMutation({
    mutationFn: () => api<MmpLink>("/links", { method: "POST", body: JSON.stringify(newLink) }),
    onSuccess: (link) => {
      setNewLink((s) => ({ ...s, name: "", destinationUrl: "" }));
      invalidateAll();
  ...
  const kpis = [
    ...
    { label: "Installs", value: summary?.installs ?? 0, icon: <Download className="w-4 h-4 text-emerald-500" />, sub: summary ? `${summary.attributedInstalls} attributed · ${summary.organicInstalls} organic` : undefined },
    { label: "Events", value: summary?.events ?? 0, icon: <Activity className="w-4 h-4 text-purple-500" /> },
    { label: "Revenue", value: `$${(summary?.revenueUsd ?? 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}`, icon: <DollarSign className="w-4 h-4 text-amber-500" /> },
  ];
      ...
      <div className="space-y-6" data-testid="page-mmp">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Attributed installs and revenue per tracking link (last-click, 7-day window).</CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <tbody>
                    {breakdown.map((row) => (
                      <tr key={row.linkId} className="border-b last:border-0" data-testid={`row-breakdown-${row.code}`}>
                        ...
                        <td className="py-2 pr-4 text-right">${row.revenueUsd.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td>
                      </tr>
                    ))}
                    {organic && (organic.installs > 0 || organic.revenueUsd > 0) && (
                      <tr className="text-muted-foreground">
                        ...
                        <td className="py-2 pr-4 text-right">—</td>
                        <td className="py-2 pr-4 text-right">${organic.revenueUsd.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <div className="flex gap-2">
                  <Select value={newLink.appId} onValueChange={(v) => setNewLink((s) => ({ ...s, appId: v }))}>
...
{`curl -X POST ${window.location.origin}/api/v1/mmp/ingest/install \\
  ...
  -d '{"deviceId": "STABLE-DEVICE-ID"}'`}
            ...
            <div>
              <Label className="text-xs text-muted-foreground uppercase tracking-wide">2 — Report an in-app event (with optional revenue in USD)</Label>
              <pre className="bg-muted rounded-lg p-3 mt-1 overflow-x-auto text-xs">
