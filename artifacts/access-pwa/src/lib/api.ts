...
/**
 * Root-relative API base. The shared dev proxy and the production nginx layer
 * both forward `/api` to the API server, so this works unchanged in dev and on
 * m.thea.quest. Do NOT prefix with import.meta.env.BASE_URL — that would hit
 * this app's own static server and return SPA HTML.
 */
const API_BASE = "/api";
...
async function request<T>(
  ...
  try {
    res = await fetch(`${API_BASE}${path}`, {
      ...
      headers,
      // Session is a first-party HttpOnly cookie — always send it.
      credentials: "include",
      body:
...
export const api = {
  ...
  async updateMember(
    ...
    return request<Member>(`/v1/members/${id}`, {
      method: "PATCH",
      body: input,
    });
  },
  ...
  // ── Grants ────────────────────────────────────────────────────────────────
  async listGrants(
    params: { memberId?: string; accessPointId?: string } = {},
  ): Promise<Grant[]> {
  ...
  async createGrant(input: {
    ...
    return request<Grant>("/v1/access/grants", {
      method: "POST",
      body: input,
    });
  },
