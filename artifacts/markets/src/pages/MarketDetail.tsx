...
import { ArrowLeft, Clock, CheckCircle2, TrendingUp, BarChart3, AlertCircle, Info, Share2 } from "lucide-react";
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="glass-panel rounded-2xl p-6 md:p-10 border border-primary/20 shadow-2xl shadow-primary/5">
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <span className="inline-flex items-center rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-sm font-semibold text-primary uppercase tracking-wide">
              {market.category}
            </span>
            ...
            <div className="ml-auto">
              <Button variant="ghost" size="sm" onClick={handleShare} className="text-muted-foreground hover:text-white border-primary/20 hover:bg-primary/20">
              ...
              return (
                <div 
                  ...
                  className={`relative overflow-hidden rounded-xl bg-secondary/30 border transition-all duration-300 ${
                    isSelected ? "border-primary shadow-[0_0_15px_rgba(59,130,246,0.2)]" : 
                    isWinner ? "border-green-500 shadow-[0_0_15px_rgba(34,197,94,0.2)]" : 
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`flex items-center justify-center w-8 h-8 rounded-full border-2 ${
                        isWinner ? "border-green-500 bg-green-500/20 text-green-400" :
                        ...
                        {isWinner ? <CheckCircle2 className="w-5 h-5" /> : <span className="font-mono font-bold">{i+1}</span>}
                      </div>
                      <span className={`text-xl font-medium ${isWinner ? 'text-green-400' : isSelected ? 'text-primary font-bold' : 'text-foreground'}`}>
                        {opt.label}
                        {isSelected && <span className="ml-3 text-sm opacity-75 font-normal tracking-wide uppercase">(Your Pick)</span>}
                      </span>
                    </div>
                    ...
                    {showResults ? (
                      ...
                      <div className={`px-4 py-2 rounded font-semibold uppercase tracking-wider text-sm transition-all ${
                        isVoting ? "opacity-0" : "bg-primary/20 text-primary group-hover:bg-primary group-hover:text-primary-foreground"
                      }`}>
                        Vote
                      </div>
                    )}
          ...
          <div className="flex flex-wrap items-center gap-6 pt-6 border-t border-border/50 text-sm text-muted-foreground">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-white text-base">{market.totalVotes.toLocaleString()}</span> Total Votes
            </div>
