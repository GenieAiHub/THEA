...
function buildHtml(template: string, data: Record<string, unknown>): string {
  switch (template) {
    ...
    case "narrative-alert": {
      const { orgName, entity, severity, sentimentShift, currentSentiment, dashboardUrl } = data as Record<string, string>;
      const color = severity === "critical" ? "#e53e3e" : severity === "high" ? "#dd6b20" : "#3182ce";
      return `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="font-family:sans-serif;background:#f8fafc;margin:0;padding:24px"><div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.1)"><div style="background:${color};padding:24px;color:#fff"><h1 style="margin:0;font-size:22px">🤖 THEA AI Narrative Alert</h1><p style="margin:4px 0 0;opacity:.9">${orgName}</p></div><div style="padding:24px"><p style="font-size:16px;color:#334155;margin:0 0 16px">AI assistants are describing <strong>${entity}</strong> significantly more negatively than before.</p><table style="border-collapse:collapse;width:100%"><tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Entity</td><td style="padding:8px">${entity}</td></tr><tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Severity</td><td style="padding:8px">${(severity ?? "").toUpperCase()}</td></tr><tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Sentiment shift</td><td style="padding:8px">${sentimentShift}</td></tr><tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Current sentiment</td><td style="padding:8px">${currentSentiment}</td></tr></table><div style="margin-top:24px"><a href="${dashboardUrl}" style="background:${color};color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;font-weight:600">View AI Narrative</a></div></div><div style="padding:16px 24px;background:#f8fafc;font-size:12px;color:#64748b">Powered by THEA Intelligence</div></div></body></html>`;
    }
    case "sov-alert": {
      const { orgName, entity, severity, sovShift, overtakenBy, dashboardUrl } = data as Record<string, string>;
      const color = severity === "critical" ? "#e53e3e" : severity === "high" ? "#dd6b20" : "#3182ce";
      const headline = overtakenBy
        ? `<strong>${overtakenBy}</strong> has overtaken <strong>${entity}</strong> in AI share of voice.`
        : `<strong>${entity}</strong>'s AI share of voice dropped significantly since the last check.`;
      return `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="font-family:sans-serif;background:#f8fafc;margin:0;padding:24px"><div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.1)"><div style="background:${color};padding:24px;color:#fff"><h1 style="margin:0;font-size:22px">📉 THEA AI Share of Voice Alert</h1><p style="margin:4px 0 0;opacity:.9">${orgName}</p></div><div style="padding:24px"><p style="font-size:16px;color:#334155;margin:0 0 16px">${headline}</p><table style="border-collapse:collapse;width:100%"><tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Brand</td><td style="padding:8px">${entity}</td></tr><tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Severity</td><td style="padding:8px">${(severity ?? "").toUpperCase()}</td></tr><tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Share of voice</td><td style="padding:8px">${sovShift}</td></tr>${overtakenBy ? `<tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Overtaken by</td><td style="padding:8px">${overtakenBy}</td></tr>` : ""}</table><div style="margin-top:24px"><a href="${dashboardUrl}" style="background:${color};color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;font-weight:600">View AI Narrative</a></div></div><div style="padding:16px 24px;background:#f8fafc;font-size:12px;color:#64748b">Powered by THEA Intelligence</div></div></body></html>`;
    }
    case "digest": {
    ...
    case "sighting-alert": {
      const { orgName, targetName, targetType, source, matchType, confidence, detail, seenAt, dashboardUrl } = data as Record<string, string>;
      return `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="font-family:sans-serif;background:#f8fafc;margin:0;padding:24px"><div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.1)"><div style="background:#dc2626;padding:24px;color:#fff"><h1 style="margin:0;font-size:22px">👁 THEA Security Watch — Sighting</h1><p style="margin:4px 0 0;opacity:.9">${orgName}</p></div><div style="padding:24px"><p style="font-size:18px;font-weight:600;margin:0 0 16px">Target spotted: <strong>${targetName}</strong></p><table style="border-collapse:collapse;width:100%"><tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Target type</td><td style="padding:8px">${targetType}</td></tr><tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Source</td><td style="padding:8px">${source}</td></tr><tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Match</td><td style="padding:8px">${matchType}${detail ? ` — ${detail}` : ""}</td></tr><tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Confidence</td><td style="padding:8px">${confidence}</td></tr><tr><td style="padding:8px;background:#f1f5f9;font-weight:600">Seen at</td><td style="padding:8px">${seenAt}</td></tr></table><div style="margin-top:24px"><a href="${dashboardUrl}" style="background:#dc2626;color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;font-weight:600">View Sighting</a></div></div><div style="padding:16px 24px;background:#f8fafc;font-size:12px;color:#64748b">Powered by THEA Security Watch</div></div></body></html>`;
    }
    default:
